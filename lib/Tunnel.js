...
module.exports = class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { host, port: local_port, local_host, query_params } = this.opts;
  ...
  _init(cb) {
    ...
    const queryparams = typeof opt.query_params === 'object' ? '?' + Object.keys(opt.query_params).map(key => key + '=' + opt.query_params[key]).join('&') : ''

    const baseUri = `${opt.host}/`;
    // no subdomain at first, maybe use requested domain
    const assignedDomain = opt.subdomain;
    // where to quest
    const uri = baseUri + (assignedDomain || '?new') + queryparams;
